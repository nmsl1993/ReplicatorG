...
public class HPGLDriver extends SerialDriver implements PenPlotter {
	/**
	...
	public void initialize() {
		...
		if (!isInitialized()) {
			try {
				...
				sendHPGLCommand("IN;SP1;");
				
				Base.logger.info("Initializing Serial.");
				while (!isInitialized()) {
					// TODO: Is there an init string here? We just say, now initialized!
					setInitialized(true);
	...
	protected void sendCommand(String next) {
		...
		Base.logger.fine("dropping: " + next);
...
//		// commands)");
	 ...
	 * method will block until the command has been sent.
	 */
	protected void sendHPGLCommand(String next) {
//		assert (isInitialized());
		assert (serial != null);
		// System.out.println("sending: " + next);
...
//		sendCommand(cmd);
		...
		// Note that we ignore everything but x&y
		String cmd = "PA" + (int)((p.x()+100)*46) + "," + (int)((p.y()+100)*46) + ";"; 
		sendHPGLCommand(cmd);
